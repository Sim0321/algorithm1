...
function solution(park, routes) {
  ...
  let startRow = park.findIndex((el) => el.includes("S"));
  let startCol = park[startRow].indexOf("S");

  routes.forEach((postion) => {
    const [direction, num] = postion.split(" ");
...
// 해설 및 다른 사람 풀이
// 처음엔 문제를 이해하지 못했다. 이유는 공원의 길이를 파악하지 못했었다. 질문을 자세히 보니 가로의 길이는 W, 세로 길이는 H, 좌측 상단, 우측 하단은 각각 H-1, W-1 이다. 예를 들어 W가 3이면 높이는 2이다.
